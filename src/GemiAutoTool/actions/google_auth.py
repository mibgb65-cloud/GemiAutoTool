...
from GemiAutoTool.utils.selenium_util import wait_and_type, wait_and_click, is_element_exist, random_sleep


def login_google(driver, account, task_name: str) -> bool:
    """执行极简版的 Google 登录流程"""
    print(f"[{task_name}] 正在执行登录流程: {account.email}")

    # 0. 访问 Google 首页并点击右上角的登录按钮
    driver.get("https://www.google.com")
    print(f"[{task_name}] 已打开 Google 首页，寻找登录按钮...")
    ...
    if not wait_and_click(driver, By.CSS_SELECTOR, login_btn_locator, timeout=10, task_name=task_name):
        print(f"[{task_name}] ❌ 未能在首页找到登录按钮，请检查页面是否加载完全。")
        return False

    print(f"[{task_name}] ✅ 成功点击首页登录按钮，正在进入账号密码输入页...")
    time.sleep(3)  # 等待页面跳转
    ...
    if code and is_element_exist(driver, By.CSS_SELECTOR, totp_locator, timeout=8):
        print(f"[{task_name}] 成功生成 2FA: {code}，正在输入...")
        ...
        print(f"[{task_name}] ℹ️ 账号无有效 2FA 密钥，直接跳过 2FA 步骤。")
    ...
    if is_element_exist(driver, By.XPATH, skip_xpath, timeout=5):
        print(f"[{task_name}] ↗️ 发现推广弹窗，正在跳过...")
    ...
    if "myaccount.google.com" in current_url or "google.com" in current_url:
        print(f"[{task_name}] ✅ 登录成功")
        return True

    print(f"[{task_name}] ℹ️ 登录后停留页面未识别: {current_url[:50]}...")
    return False